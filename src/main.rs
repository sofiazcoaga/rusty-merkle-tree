use crate::merkle_tree::{create_merkle_tree_from_data, verify_element, MerkleTree};

pub mod merkle_tree;
pub fn main() {
    // Create a Merkle Tree with 3 data elements will generate a tree with 4 leaves
    let my_data = vec![b"Hello".to_vec(), b"i am a".to_vec(), b"Merkle Tree".to_vec()];
    for d in my_data.clone() {
        println!("\nFor data: {:?} the hash is: {:?}", String::from_utf8(d.clone()).unwrap(), sha256::digest(d));
    }

    // Create Merkle Tree
    let mut merkle_tree = create_merkle_tree_from_data(&my_data);
    println!("\n The initial Merkle tree is (hashes can be verified in leaves): ");
    print_three_level_tree(&merkle_tree);

    println!("Now let's add the element {:?} with hash {:?}", "!", sha256::digest(b"!".to_vec()));

    // Add an element
    merkle_tree.add_element(b"!".to_vec());
...
fn print_three_level_tree(tree: &MerkleTree) {
    ...
    println!("      {:?}              {:?}    \n", &tree.get_tree_element(1, 0)[..8], &tree.get_tree_element(1, 1)[..8]);
    println!(" {:?} {:?}   {:?} {:?} \n", &tree.get_tree_element(2, 0)[..8], &tree.get_tree_element(2, 1)[..8], &tree.get_tree_element(2, 2)[..8], &tree.get_tree_element(2, 3)[..8]);
}
