...
impl MerkleTree {
    ...
    /// Returns the index of the last leaf with real data.
    ...
    /// Returns the amount of leaves the Merkle Tree has.
    ...
    /// Sets the index of the last element that has real data. Used
    /// to update the tree when a new element is added.
    ...
    ///
    pub fn add_element(&mut self, new_element: Vec<u8>) -> Result<(), MerkleTreeError> {
...
mod test {
    ...
    fn add_element_in_existing_position() {
        ...
        let add_element_result = merkle_tree.add_element(b"128746124".to_vec());
        assert!(add_element_result.is_ok());
